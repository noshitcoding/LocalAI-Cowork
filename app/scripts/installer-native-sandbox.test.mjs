...
test('the Windows release smoke exercises the installed native sandbox', () => {
  ...
  assert.match(nativeSandbox, /#\[cfg\(debug_assertions\)\][\s\S]*LACOWORK_NATIVE_SANDBOX_SKIP_SETUP/)
  assert.match(nativeSandbox, /#\[cfg\(not\(debug_assertions\)\)\][\s\S]*let skip_setup = false/)
  assert.match(nativeSandbox, /setup_start\(app_data\)/)
  assert.match(nativeSandbox, /let repeated = setup\(\)\?/)
