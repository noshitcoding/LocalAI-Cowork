...
test('the Windows release smoke exercises the installed native sandbox', () => {
  ...
  assert.match(nativeSandbox, /password_wide\.as_ptr\(\),\s*LOGON_WITH_PROFILE,/)
  assert.match(nativeSandbox, /format!\("APPDATA=\{profile\}\\\\AppData\\\\Roaming"\)/)
  assert.match(nativeSandbox, /format!\("LOCALAPPDATA=\{profile\}\\\\AppData\\\\Local"\)/)
  assert.match(nativeSandbox, /timeout_ms: Some\(120_000\)/)
