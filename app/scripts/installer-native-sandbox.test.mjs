...
test('the Windows release smoke exercises the installed native sandbox', () => {
  ...
  assert.match(nativeSandbox, /let logon_sid = match process_logon_sid\(process\.hProcess\)/)
  assert.match(nativeSandbox, /apply_run_access_plan\(&capability_key, &logon_sid\)/)
  assert.match(nativeSandbox, /let _run_acl_guard = RunAclGuard/)
  assert.match(nativeSandbox, /create_capability_restricted_token\(token, &request\.capability_sid\)/)
  ...
  assert.doesNotMatch(nativeSandbox, /fn capability_sid_from_hash/)
