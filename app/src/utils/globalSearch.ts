...
const STATIC_ENTRIES: SearchIndexEntry[] = [
  ...
  },
  {
    id: "setting-runs",
    type: "setting",
    title: "Runs and Memory",
    subtitle: "Chat context, run history, and memory",
    body: "Runs Chats Memory Context history",
    href: "/settings",
    action: "open-settings",
    targetId: "runs",
  },
...
export function buildSearchIndex({
  ...
  tasks,
  projects,
}: BuildSearchIndexInput): SearchIndexEntry[] {
  ...
  for (const thread of threads) {
    ...
    const messageBody = visibleMessages
    ...
    entries.push({
      ...
      body: compact([
        ...
        ...project.resources.map((resource) =>
          compact([resource.label, resource.path, resource.kind]),
        ),
      ]),
      href: "/projects",
      action: "open-project",
