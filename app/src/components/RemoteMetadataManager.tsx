...
function GuidedMetadataFields({ type, payload, onChange }: GuidedFieldsProps) {
  ...
  return <>
    ...
    <label>Preferred transport<select aria-label="MCP metadata transport" value={stringOf(payload.transport, 'stdio')} onChange={(event) => setField('transport', event.target.value)}><option value="stdio">stdio</option><option value="streamable_http">Streamable HTTP</option><option value="sse">Legacy HTTP+SSE</option></select></label>
