...
import type { CapabilityCatalog, ExecutorTarget, ProjectRecord, RunRecord } from '../runtime/contracts'
...
function targetChoices(catalog: CapabilityCatalog): TargetChoice[] {
  ...
  for (const executor of catalog.executors) {
    ...
    const names = new Set(executor.registration.capabilities.map((capability) => capability.name))
...
export default function RemoteRunComposer({
  ...
  const choices = useMemo(() => catalog ? targetChoices(catalog) : [], [catalog])
